  ...
  return (
    <div className="glass-panel p-6 rounded-xl space-y-6">
      <div className="flex items-center gap-2 border-b border-slate-200 pb-4">
        ...
        <h2 className="font-semibold text-base text-slate-900">Email Senders</h2>
      </div>
      ...
      {successMsg && (
        <div className="flex items-start gap-2 p-3 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs">
      ...
      <form onSubmit={handleAddSender} className="space-y-3">
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full flex items-center justify-center gap-1.5 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-xs font-bold text-white transition-all disabled:opacity-50 cursor-pointer shadow-sm active:scale-[0.98]"
        >
          {submitting ? (
      ...
      <div className="space-y-2 pt-2 border-t border-slate-200">
        ...
        {loading ? (
          ...
          <p className="text-xs text-slate-450 text-center py-3 italic font-medium">
          ...
          <div className="max-h-56 overflow-y-auto space-y-2 pr-1">
            {senders.map((s) => (
              <div
                key={s.id}
                className="flex items-center justify-between p-3 rounded-xl border border-slate-200 bg-slate-50 hover:border-slate-300 hover:bg-slate-100 transition-all duration-200 text-xs"
              >
                <div>
                  <div className="font-semibold text-slate-800">{s.name}</div>
                  <div className="text-slate-500 font-mono text-[11px] mt-0.5">{s.email}</div>
                </div>
                <button
                  onClick={() => handleDeleteSender(s.id)}
                  className="p-1.5 rounded bg-rose-50 border border-rose-200 text-rose-600 hover:bg-rose-600 hover:text-white transition-all cursor-pointer"
                  title="Remove Sender"
                >
